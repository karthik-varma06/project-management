        ...
        <div>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
                {["status", "type", "priority", "assignee"].map((name) => {
                    ...
                    return (
                        <select key={name} name={name} onChange={handleFilterChange} className="border bg-white dark:bg-slate-900 border-zinc-300 dark:border-zinc-700 outline-none px-3 py-1 rounded text-sm text-zinc-900 dark:text-gray-200 dark:hover:bg-slate-800 transition-colors" >
                                        ...
                                        return (
                                            <tr key={task.id} onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                                                ...
                                                <td onClick={e => e.stopPropagation()} className="px-4 py-2">
                                                    <select name="status" onChange={(e) => handleStatusChange(task.id, e.target.value)} value={task.status} className="group-hover:ring ring-zinc-100 dark:ring-zinc-700 bg-white dark:bg-slate-900 outline-none px-2 pr-4 py-1 rounded text-sm text-zinc-900 dark:text-gray-200 border border-transparent dark:border-zinc-700 dark:hover:bg-slate-800 cursor-pointer transition-colors" >
